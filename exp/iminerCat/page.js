...
var page = {
	...
	addMessageListener: function() {
		chrome.extension.onRequest.addListener(function(c, b, a) {
			...
			log("in page.js onRequest, request.msg=" + c.msg);
			switch (c.msg) {
				case "capture_window":
...
//////////
		...
		chrome.storage.local.get(null,function(item){
			// var min = item.min
			// var max = item.max
			page.endX = page.getElementLeft(b) + parseInt(item.min,10);
			page.endY = page.getElementTop(b) + parseInt(item.max,10);
			b.style.width = item.min + 'px';
			b.style.height = item.max + 'px';
		})
