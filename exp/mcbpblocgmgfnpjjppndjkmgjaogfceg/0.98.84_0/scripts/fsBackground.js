	  ...
	  function(request, sender, sendResponse) {
		...
		switch (request.message)
		{
			...
			case "loadScript":
				...
				chrome.tabs.executeScript(tabId, {file:"scripts/fsUtils.js", runAt: "document_start"},
					function() {
						chrome.tabs.executeScript(tabId, {file:"scripts/fsSelection.js", runAt: "document_start"},
							function() {
                                chrome.tabs.executeScript(tabId, {file:"scripts/fsLinks.js", runAt: "document_start"},
                                    function() {
                                        chrome.tabs.executeScript(tabId, {file:"scripts/fsContent.js", runAt: "document_start"},
                                            function(){
...
function getMenuSettings(callback)
{
	chrome.windows.getLastFocused(function(window) {
		chrome.tabs.getSelected(window.id, function(tab) {
			chrome.tabs.executeScript(tab.id, {code:"{}", runAt: "document_start"}, function () {
...
function capturePage(Action, Mode, CallbackCompleted)
{
    ...
    chrome.windows.getLastFocused(function(window) {    //获得最近获得焦点的窗口，通常为顶层窗口
		...
		chrome.tabs.getSelected(null, function(tab)	{ // 获取当前tab
			...
			chrome.tabs.executeScript(tab.id, {code:"{}", runAt: "document_start"}, function () {
				...
				else
				{
                    ...
                    chrome.tabs.executeScript(tabId, {file: "scripts/fsScriptChecker.js", runAt: "document_start"}, function() {
	...
	chrome.windows.getLastFocused(function(window) {
		chrome.tabs.getSelected(window.id,
			function(tab)
			{
				...
				try {
                    var port = chrome.tabs.connect(tabId, {name: commPortName}), connecting = true;

                    port.postMessage({topic: "init", mode: Mode, debug: isDebug, p: localStorage[cPluginProModePref] == "true"});
	...
	chrome.contextMenus.removeAll(
		function()
		{
			...
			if (!guiItemsLocked)
			{
				...
				mnuFireShotNative = !fLite || chrome.contextMenus.create({title: chrome.i18n.getMessage("action_install_native") + "...", parentId: mnuRoot, onclick: genericOnClick});
                    ...
                    chrome.tabs.query({windowId: window.id}, function (tabs) {
                        var currentTab = 0,
                            cycle = new AsyncCycle(function (iterator) {
                                    try {
                                        chrome.tabs.highlight({windowId: window.id, tabs: currentTab++}, function () {
