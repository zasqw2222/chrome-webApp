    ...
    chrome.windows.getLastFocused(function (window) {
        ...
        chrome.tabs.getSelected(null, function (tab) {
            ...
            chrome.tabs.executeScript(tab.id, {
                ...
                // Mode 2
                // cModeBrowser 3
                if (Mode == cModeBrowser && (noExecScript || (tab.url !== undefined && tab.title !== undefined))) //区域截图 不走这里
                    setTimeout(
                        function () {
...
function doCapturing(Action, Mode, CallbackCompleted) {
    ...
    if (capId <= processedId++) {//第一次 不会走这里
        capId = processedId;
    ...
    chrome.windows.getLastFocused(function (window) {
        ...
        chrome.tabs.getSelected(window.id,
            function (tab) {
                ...
                try {
                    // 连接到指定标签页中的内容脚本，这样当前应用在指定标签页中运行的每一个内容脚本都会产生 runtime.onConnect 事件
                    // commPortName 内容脚本名称
                    // alert(commPortName)
                    // 应用 向 内容页面发送消息 返回的是双向通信的 runtime.port  长时间链接
                    var port = chrome.tabs.connect(tabId, {
                        name: commPortName
                    }),
                        connecting = true;//连接 
                    //长连接的时候 runtime.port 发送消息的方法
                    port.postMessage({
                        topic: "init",
                        mode: Mode,
                        debug: isDebug,
